...
class ItGlueClient
{
    ...
    protected function buildDomainTraits(Domain $domain, array $dnsRecords): array
    {
        ...
        if ($whoisKey = $this->traitKey('whois')) {
            $traits[$whoisKey] = $this->formatWhoisData($domain);
        }
    ...
    /**
     * Format WHOIS data from IP2WHOIS for flexible asset traits
     */
    protected function formatWhoisData(Domain $domain): string
    {
        if (empty($domain->whois_data) || !is_array($domain->whois_data)) {
            return 'WHOIS data not available. Sync from IP2WHOIS to populate this field.';
        }
        ...
        // Nameservers
        if (isset($whoisData['nameservers']) && is_array($whoisData['nameservers']) && !empty($whoisData['nameservers'])) {
            $formatted .= "Nameservers:\n";
            foreach ($whoisData['nameservers'] as $ns) {
                $formatted .= "  - {$ns}\n";
            }
            $formatted .= "\n";
        }
